...
<span style="color: rgb(255, 76, 65);">**⚠️特别提示:** </span> 请先下载本教程需要用到的KidsBlock(Scratch)资料(包含：KidsBlock(Scratch)_代码等）和 Android_APP 等，保存至您方便使用的路径下。

**下载：**  [KidsBlock(Scratch)资料](./KidsBlock(Scratch)资料.7z) 和 [Android_APP](./Android_APP.7z)

## 8.2. KidsBlock IDE开发环境设置

### 8.2.1. 下载和安装 KidsBlock IDE

- 如何下载、安装及使用 KidsBlock IDE 的参考链接：[https://www.keyesrobot.cn/projects/KidsBlock/zh-cn/latest/](https://www.keyesrobot.cn/projects/KidsBlock/zh-cn/latest/)

### 8.2.2. 选择设备

（1）驱动安装完后，点击![](media/KidsBlock-IDE1.png)进入主控板页面，可以选择需要添加的设备（控制板），本项目需要选择SmarthomeESP32。先点击SmarthomeESP32所处区域，后点击“连接”。这样SmarthomeESP32已连接上，点击“返回编辑器”回到编码编辑器。这样，我们会发现![](media/KidsBlock-IDE1.png)变成![](media/KidsBlock-IDE2.png)，同时![](media/KidsBlock-IDE3.png)变成![](media/KidsBlock-IDE4.png)，说明SmarthomeESP32和端口（COM）都已经连接上了。

![](media/KidsBlock-IDE5.png)

![](media/KidsBlock-IDE6.png)

![](media/KidsBlock-IDE7.png)

![](media/KidsBlock-IDE8.png)

（2）假如SmarthomeESP32已经连接上后，但是![](media/KidsBlock-IDE1.png)没有变成![](media/KidsBlock-IDE2.png)，则需要手动点击![](media/KidsBlock-IDE1.png)来连接端口（COM）。先点击![](media/KidsBlock-IDE3.png)，在出现的对话页面中点击![](media/CONNECT1.png)，连接成功后，会出现“已连接”页面，说明端口已连接上了。

![](media/KidsBlock-IDE9.png)
...
（3）如果需要断开端口，先点击![](media/KidsBlock-IDE4.png)，在出现的对话页面中点击“断开连接”。这样，端口就断开了。

![](media/NOCONNECT.png)

![](media/KidsBlock-IDE10.png)

（4）打开已有的SB3类型文件的方法：

打开Kidsblock软件，点击“文件”，选择点击“从电脑中上传”，在电脑上选中需要打开的SB3类型文件（例如：![](media/1874abbb35ed2bb3a1bd5967f3a13181.png)文件）
...
![Img](./media/img-20250604134917.png)
...
![Img](./media/img-20250604135132.png)
...
简单的使用![](media/76ebed9bb34d03a41a2317cf6215eff8.png)作为正确密码。
...
上传测试代码成功，上电后，如果成功连接上WiFi，LCD1602 显示分配到的IP地址，同时在串口监视器点击![](media/6081c6cdc3af46ed5666bf706eac396c.png)设置波特率为9600，串口监视器会打印出分配到的IP地址。（假如串口监视器没打印出分配到的IP地址，可以按下主板上的复位键重启)
...
![Img](./media/img-20250604154159.png)

连上WIFI之后，分别单击APP上对应的按钮，串口监视器窗口打印接收到的字符串。

![Img](./media/img-20250604153359.png)
...
![Img](./media/img-20250604143643.png)

**1.6. 操作步骤及实验结果1：**

上传测试代码成功，上电后，如果成功连接上WiFi，LCD1602显示出分配到的IP地址，同时在串口监视器点击![](media/6081c6cdc3af46ed5666bf706eac396c.png)设置波特率为9600，串口监视器也会打印出分配到的IP地址。（假如串口监视器没打印出分配到的IP地址，可以按下主板上的复位键重启)
...
APP输入IP地址（LCD1602显示出分配到的IP地址或在串口监视器点击![](media/6081c6cdc3af46ed5666bf706eac396c.png)设置波特率为9600，串口监视器会打印出分配到的IP地址），点击 **CONNECT** 连接，连接成功标志是APP下方会跳出 Connected，需要注意看.
...
![Img](./media/img-20250604143449.png)
...
A. APP输入IP地址（LCD1602显示出分配到的IP地址，或点击IDE的串口监视器![](media/6081c6cdc3af46ed5666bf706eac396c.png)设置波特率为9600，串口监视器窗口显示的IP地址）
